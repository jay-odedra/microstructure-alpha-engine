...
for i in tqdm(range(50000)):
    ...
    if len(depth_buffer) >= buffer_size:
        pd.DataFrame(depth_buffer).to_parquet(
            f"E:\\Quant_Projects\\microstructure-alpha-engine\\microstructure-alpha-engine\\data\\raw\\lob\\lob_data_raw_{i}.parquet",
            compression="snappy",
        )
        pd.DataFrame(trades_buffer).to_parquet(
            f"E:\\Quant_Projects\\microstructure-alpha-engine\\microstructure-alpha-engine\\data\\raw\\trades\\trades_data_raw_{i}.parquet",
            compression="snappy",
        )
        depth_buffer = []
        trades_buffer = []
